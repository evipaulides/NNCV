...
def get_svdd_radius(dist_sq: torch.Tensor, nu: float) -> float:
    """Compute radius R as the (1-nu)-quantile of squared distances."""
    return float(torch.quantile(dist_sq, 1 - nu).item())


def extract_bottleneck_features(model, images):
    """Run encoder forward pass and return mean-pooled bottleneck features."""
    x1 = model.inc(images)
    x2 = model.down1(x1)
    x3 = model.down2(x2)
    x4 = model.down3(x3)
...
def fit_svdd(model, dataloader, device, nu=0.1, n_epochs=10, lr=1e-4, warm_up_epochs=3):
    """
    Fit a Deep SVDD hypersphere on the U-Net encoder bottleneck features.
    """
    # Step 1: Initialise center c
    ...
    c = all_features.mean(dim=0)

    # Avoid trivial collapse: push near-zero coordinates away from 0
    eps = 0.1
    c[(c.abs() < eps) & (c < 0)] = -eps
    c[(c.abs() < eps) & (c > 0)] = eps
    c = c.to(device)
...
def main(args):
    ...
    print("Fitting Deep SVDD hypersphere")
