...
class Model(nn.Module):
    ...
    def __init__(
        ...
        # Deep SVDD hypersphere buffers
        self.register_buffer("svdd_center", torch.zeros(512))
        self.register_buffer("svdd_radius", torch.tensor(0.0))
    ...
    def svdd_score(self, feat):
        ...
        dist_sq = torch.sum((feat - self.svdd_center.unsqueeze(0)) ** 2, dim=1)
        return dist_sq - self.svdd_radius ** 2
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_p=0.0):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(in_channels, out_channels, dropout=dropout_p)
        )
