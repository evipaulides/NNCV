...
def postprocess(pred: torch.Tensor, original_shape: tuple) -> np.ndarray:
    ...
    pred_max = torch.argmax(pred, dim=1, keepdim=True)  # Get the class with the highest probability
...
def main():
    ...
    with torch.no_grad():
        for img_path in image_files:
            ...
            seg_pred, svdd_score = model(img_tensor)

            include_decision = (svdd_score < 0).item()  # Convert to Python bool
